...
class Camunda:
    ...
    def call(self, method, url, payload, params):
        res = requests.request(method=method, url=url, data=payload, params=params)
        return res

    def processes(self):
        _logger.info('getting processes')
        data = self.get('process-definition', params={'active':'true', 'latestVersion':'true'})
